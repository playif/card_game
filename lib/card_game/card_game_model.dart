...
class Visible {
  ...
  Visible._fromJson(data)
      : this._(data['showCard'], showNum: data['showNum'], max: data['max'],
          min: data['min']);
...
class GameModel {
  ...
  static Map<AITagger, Map<int, double>> s_prob = new Map<AITagger, Map<int,
      double>>();
  final List<Map<AITagger, Map<int, double>>> _prob = new List<Map<AITagger,
      Map<int, double>>>();
...
class DeckModel {
  ...
  int get maxShow => _visible.max == 0 ? _cards.length : min(_cards.length,
      _visible.max);
