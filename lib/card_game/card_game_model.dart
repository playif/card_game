...
class Visible{
  final int showCard; //0:自己看不到  1:自己看的到 2:別人看的到
  final int showNum; //0:自己看不到  1:自己看的到 2:別人看的到
...
//  final List<CardDef> _cardDef=new List<CardDef>();
  ...
  final UserModel _host=new UserModel._(HOST);
...
//        int cid=data['cid'];
//        //print(set);
//        _cardDef.add(new CardDef(set, cid));
//        break;
      case 'setup':
        ...
        _id=data['uid'];
        //print(data);
        _rand=new Random(seed); //use common rand seed;
        //print("setup");
        for(int i=0;i<users;i++){
          UserModel user=new UserModel._(i);
          _users.add(user);
        }
        break;
      case 'start':
      ...
      case 'newDeck':
        int uid=data['uid'];
        //String type=
        DeckModel deck=new DeckModel._(uid,new Visible._fromJson(data['vis']));
      ...
      case 'insertCard':
        ...
        int did=data['did'];
        int pos=data['pos'];
        int cid=data['cid']; //card Info index  
        int set=data['set'];
      ...
      case 'removeCard':
        int uid=data['uid'];
        int did=data['did'];
        int pos=data['pos']; //card Info index
      ...
      case 'moveCard':
        int uid1=data['uid1'];
        int did1=data['did1'];
        int pos1=data['pos1']; //card pos index
        int uid2=data['uid2'];
        int did2=data['did2'];
        int pos2=data['pos2']; //card pos index
...
class DeckModel {
  ...
  DeckModel._(this._uid,this._visible);
...
//  List<CardModel> lastCards([max=100]){
//    return _cards.reversed.take(max).toList();
//  }
