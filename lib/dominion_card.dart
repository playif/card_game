...
class CardTypeCondition extends Condition{
  ...
  bool test(Command command){
    if(command is CardCommand){
      return CardSet.def(command.cid).types[type];//command.did==CardDef;
    }
    return false;
  }
}
  ...
  bool test(Command command){
    ...
    if(command is CardCommand){
      ...
      return CardSet.def(command.cid).cost<=maxCost &&
          CardSet.def(command.cid).cost>=minCost;//command.did==CardDef;
    }
    return false;
  }
}
  ...
  bool test(Command command){
    if(command is CardCommand){
      return CardSet.def(command.cid).cost<=model.getValue(GOLD) &&
          model.getValue(BUY)>=1;//command.did==CardDef;
    }
    return false;
  }
}
...
//  });
  ...
  static final Condition REVEALS= new DeckCondition(REVEAL);
...
class CardSet{
  ...
  static final basic=[
    ...
    new DominionCardDef("Estate", 2, "", CardType.VICTORY,(game,cmd){},score:1),
    new DominionCardDef("Duchy", 5, "", CardType.VICTORY,(game,cmd){},score:3),
    new DominionCardDef("Province", 8, "", CardType.VICTORY,(game,cmd){},score:6,cardNum: 8),        
    new DominionCardDef("Curse", 0, "", CardType.CURSE,(game,cmd){},score:-1),
  ];
...
//      game.pushState('Cellar', "地窖--請選擇要捨棄的牌" ,Conds.SELF_HAND,btns:['結束']);
//      bool end(Command cmd){
//        game.loop(draw, (i){
//          game.drawCard(cmd.sender);
//        });
//        return true;
//      }
//      game.on("Cellar", (Command cmd){
//        if(cmd is CardCommand){
//          game.discardCard(cmd.uid, cmd.pos);
//          draw++;
//          if(game.model[cmd.uid][HAND].cardNum==0){
//            return end(cmd);
//          }
//        }
//        else if(cmd is ButtonCommand){
...
//      });
    ...
    new DominionCardDef("Chancellor", 3, "", CardType.ACTION,(DominionGame game,Command cmd){
      ...
      state.action((ButtonCommand cmd){
        if(cmd.bid==0){
          game.moveAllCard(cmd.sender, DRAW, cmd.sender, TRUNK);
        }
        state.leave();
      });
      game.pushState(state);
...
//          }
    ...
    new DominionCardDef("Spy", 4, "", CardType.ACTION_ATTACK,(DominionGame game,CardCommand cmd){
      ...
      game.loop(game.model.userNum, (uid){
        ...
        //if(uid == cmd.sender) return;
        State reveal=game.createAttackState(uid, "間諜--你的抽牌堆將被翻開一張。",invoke: uid!=cmd.sender);
        reveal.init((s){
    ...
    new DominionCardDef("Throne_Room", 4, "", CardType.ACTION,(DominionGame game,Command cmd){
      ...
      state.check((e){
        if(!game.hasType(cmd.sender, CardType.ACTION)){
          state.leave();
        }
      });
      state.action((CardCommand cmd){
    ...
    new DominionCardDef("Library", 5, "", CardType.ACTION,(DominionGame game,Command cmd){
      ...
      state.check((e){
        ...
        if(!game.checkDraw(cmd.sender) || game.model[cmd.sender][HAND].cardNum >=7){
          state.leave();
        ...
        chose.action((ButtonCommand btn){
          ...
          if(btn.bid==1){
            game.moveCard(cmd.sender, HAND, LAST_CARD, cmd.sender, REVEAL);
          }
          if(game.model[cmd.sender][HAND].cardNum >=7 || !game.model[cmd.sender][DRAW].hasCard){
            state.leave();
          }
          chose.leave();
        });
      ...
      state.end((e){
        game.endReveal(cmd.sender, TRUNK);
      });
    ...
    new DominionCardDef("Mine", 5, "", CardType.ACTION,(DominionGame game,CardCommand cmd){
      ...
      pick.check((e){
        if(game.model[cmd.sender][cmd.did].cardNum==0){
          pick.leave();
        }
        if(!game.hasType(cmd.sender, CardType.TRESURE)){
          pick.leave();
        }
      });
      pick.action((cmd){
    ...
    new DominionCardDef("Adventurer", 6, "", CardType.ACTION,(DominionGame game,Command cmd){
      ...
      state.init((e){
        while(true){
          if(!game.checkDraw(cmd.sender) || treasure >=2){
            state.leave();
            return;
          }
          game.drawCard(cmd.sender, 1);
          var doc=CardSet.def(game.model[cmd.sender][HAND][LAST_CARD].cid);
  ...
  static final prosperity=[
    ...
    new DominionCardDef("Loan", 3, "", CardType.TRESURE,(DominionGame game,CardCommand cmd){
      ...
      state.init((e){
        game[GOLD]+=1;
        while(true){
          if(!game.checkDraw(cmd.sender)){
            game.endReveal(cmd.sender, TRUNK);
            state.leave();
            //print("hi");
            return;
          }
          game.revealCard(cmd.sender, DRAW);
          var doc=CardSet.def(game.model[cmd.sender][REVEAL][LAST_CARD].cid);
          ...
          if(doc.types[CardType.TRESURE]){
            ...
            chose.action((ButtonCommand btn){
              if(btn.bid==1){
                game.trashCard(cmd.sender, REVEAL);
              }
              else{
                game.discardCard(cmd.sender, REVEAL);
                //game.moveCard(cmd.uid, cmd.did, cmd.pos, cmd.uid, TRUNK, LAST_CARD);
              }
              game.endReveal(cmd.sender, TRUNK);
              //game.endReveal(cmd.uid, TRUNK);
